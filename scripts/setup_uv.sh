...
uv pip install --python "$PYTHON" "sglang[all]==$SGLANG_VERSION" \
  --index-strategy unsafe-best-match --prerelease=allow
uv pip uninstall --python "$PYTHON" flash-attn-4 flash_attn_4 || true
# Keep the compiler/CCCL minor aligned with torch's cu130 runtime headers.
# Newer 13.2 build tools reject the 13.0 cuda_runtime_api.h bundled by torch.
...
uv pip install --python "$PYTHON" --no-build-isolation flash-attn==2.8.3
uv pip install --python "$PYTHON" flash-linear-attention==0.4.2
# 26.6 is named by the image's CUDA-13 branch but is not published on NVIDIA's
# public index; 25.6 is the latest public MathDx wheel and supports SM90/cu13.
uv pip install --python "$PYTHON" nvidia-mathdx==25.6.0 \
...
uv pip install --python "$PYTHON" onnx onnxscript
if "$PYTHON" - <<'PY'
...
uv pip install --python "$PYTHON" tilelang==0.1.9 --find-links https://tile-ai.github.io/whl/nightly/cu128/
...
uv pip install --python "$PYTHON" --no-build-isolation "$ROOT/third_party/FlashQLA"
...
if "$PYTHON" -c 'import apex, amp_C, fused_layer_norm_cuda' >/dev/null 2>&1; then
  ...
  APEX_CPP_EXT=1 APEX_CUDA_EXT=1 APEX_PARALLEL_BUILD=8 NVCC_APPEND_FLAGS="--threads 4" \
    uv pip install --python "$PYTHON" --no-build-isolation --no-cache "$ROOT/third_party/apex"
fi
...
uv pip install --python "$PYTHON" --no-build-isolation --editable "$MEGATRON_DIR"

TMS_CUDA_MAJOR=13
export TMS_CUDA_MAJOR
fetch_archive fzyzcjy/torch_memory_saver "$TMS_COMMIT" "$ROOT/third_party/torch_memory_saver"
uv pip install --python "$PYTHON" --reinstall --no-cache --no-build-isolation "$ROOT/third_party/torch_memory_saver"
...
uv pip install --python "$PYTHON" --no-build-isolation 'nvidia-modelopt[torch]>=0.37.0'
if "$PYTHON" -c 'import sglang_router' >/dev/null 2>&1; then
  echo "SGLang router imports successfully; skipping reinstall."
else
  uv pip install --python "$PYTHON" --reinstall \
    "$GITHUB_RELEASE_PROXY/zhuzilin/sgl-router/releases/download/v0.3.2-9daabcd/sglang_router-0.3.2-cp38-abi3-manylinux_2_28_x86_64.whl"
fi
...
uv pip install --python "$PYTHON" --requirements "$SLIME_DIR/requirements.txt"
uv pip install --python "$PYTHON" --no-deps --editable "$SLIME_DIR"
uv pip install --python "$PYTHON" --no-build-isolation "$SLIME_DIR/slime/backends/megatron_utils/kernels/int4_qat"
uv pip install --python "$PYTHON" 'numpy<2' 'kernels<0.15.0'
...
if [[ "${INSTALL_FA3:-0}" == "1" ]]; then
  ...
  FLASH_ATTENTION_FORCE_BUILD=TRUE uv pip install --python "$PYTHON" --no-build-isolation \
